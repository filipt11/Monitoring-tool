...
def poll_devices():
    ...
    try:
        ...
        logger.warning("Can not establish connection with Postges DB, using cahced device list for polling")
    ...
    for device in cached_device_list:
        ...
        try:
            ...
            if not interfaces_list:
                ...
                save_polled_interface_data(device.id, device.hostname, device.ip, interfaces_list)

        except Exception as e:
            logger.error(f"Error during saving interface data for {device.hostname} | {device.ip}: {e}")      
...
def save_polled_interface_data(
    ...
    for iface in interfaces_raw:
        ...
        if admin_up == 1:
            ...
            in_bps, in_util = calculate_utilization(device_hostname, if_name, "in", in_octets, speed)
            # Liczymy dla wyjścia (OUT)
            out_bps, out_util = calculate_utilization(device_hostname, if_name, "out", out_octets, speed)
